...
import io.streamnative.pulsar.plugins.auditlog.data.AuditLogEvent;
...
public class AuditLogConsumer {
    ...
    public static void main(String[] args) throws Exception{
        PulsarClient client = PulsarClient.builder()
                ...
                .build();
        Consumer<AuditLogEvent> consumer = client.newConsumer(Schema.JSON(AuditLogEvent.class))
                .subscriptionType(SubscriptionType.Shared)
                .isAckReceiptEnabled(true)
                .enableBatchIndexAcknowledgment(true)
                .acknowledgmentGroupTime(0, TimeUnit.SECONDS)
                .topic("persistent://sn/system/audit_log_all")
                .subscriptionName("audit_sub")
                .subscribe();
        while (true) {
            // Wait for a message
            Message<AuditLogEvent> msg = consumer.receive();
            try {
                // Do something with the message
                AuditLogEvent msgData = msg.getValue();
                System.out.println(messageIdToString(msg.getMessageId()) + "Message received: "
                        + msgData.toString());
