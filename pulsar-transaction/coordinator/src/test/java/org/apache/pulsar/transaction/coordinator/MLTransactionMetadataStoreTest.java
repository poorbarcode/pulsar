...
public class MLTransactionMetadataStoreTest extends MockedBookKeeperTestCase {
    ...
    public void testDeleteLog(TxnLogBufferedWriterConfig txnLogBufferedWriterConfig) throws Exception {
        ...
        MLTransactionMetadataStore transactionMetadataStore =
                new MLTransactionMetadataStore(transactionCoordinatorID, mlTransactionLog,
                        ...
                        new TransactionMetadataStoreConfig(0L, 0L, 100_000L));
        transactionMetadataStore.init(new TransactionRecoverTrackerImpl()).get();
        int checkReplayRetryCount = 0;
        while (true) {
            if (checkReplayRetryCount > 3) {
    ...
    @Test(dataProvider = "bufferedWriterConfigDataProvider")
    public void testRecoverEndedTxnStatus(TxnLogBufferedWriterConfig txnLogBufferedWriterConfig) throws Exception {
        ManagedLedgerFactoryConfig factoryConf = new ManagedLedgerFactoryConfig();
        factoryConf.setMaxCacheSize(0);
        ...
        Awaitility.await().until(transactionMetadataStore::checkIfReady);
        TxnID txnID = transactionMetadataStore.newTransaction(1000, null).get();
        transactionMetadataStore.updateTxnStatus(txnID, TxnStatus.ABORTING, TxnStatus.OPEN, true).get();
        transactionMetadataStore.updateTxnStatus(txnID, TxnStatus.ABORTED, TxnStatus.ABORTING, true).get();
        assertEquals(transactionMetadataStore.getEndedTxnStatus(txnID), EndedTxnStatus.TIMEOUT);

        transactionMetadataStore.closeAsync().get(2, TimeUnit.SECONDS);
        Assert.assertNull(transactionMetadataStore.getEndedTxnStatus(txnID));

        MLTransactionLogImpl recoveredTxnLog = new MLTransactionLogImpl(transactionCoordinatorID, factory,
                managedLedgerConfig, txnLogBufferedWriterConfig, transactionTimer, DISABLED_BUFFERED_WRITER_METRICS);
        ...
        try {
            recoveredStore.getTxnMeta(txnID).get();
            fail();
        } catch (ExecutionException e) {
            Assert.assertTrue(e.getCause() instanceof TransactionNotFoundException);
        }
